...
//   // Some versions of formidable expose .filepath, others .path
...
//                      VALUES (?, ?, ?, ?, ?, ?, ?)`;
//         const params = [name, address, city, state, contact, imagePathOrUrl, email_id];
...
import path from "path";
import { IncomingForm } from "formidable";
import { getPool } from "../../../lib/db.js"; // make sure path is correct
import { v4 as uuidv4 } from "uuid";
import { v2 as cloudinary } from "cloudinary";
import streamifier from "streamifier";
...
// Local upload directory (only used if Cloudinary is disabled)
const uploadDir = path.join(process.cwd(), "public", "schoolImages");
if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });
